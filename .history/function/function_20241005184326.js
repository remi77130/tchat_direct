...
window.addEventListener('load', fetchDivs);
  ...
  let emailList = []; // Stocker les e-mails générés globalement

        function generateEmails() {
            // Liste de caractères à utiliser pour la génération
            const characters = 'abcdefghijklmnopqrstuvwxyz0123456789';
            const domains = ['gmail.com', 'yahoo.com', 'outlook.com', 'hotmail.com'];

            emailList = []; // Réinitialiser la liste avant de générer
            for (let i = 0; i < 10; i++) {
                let emailName = '';
                // Générer un nom d'e-mail de longueur aléatoire (6 à 10 caractères)
                const nameLength = Math.floor(Math.random() * (10 - 6 + 1)) + 6;
                for (let j = 0; j < nameLength; j++) {
                    emailName += characters.charAt(Math.floor(Math.random() * characters.length));
                }
                // Sélectionner un domaine aléatoire
                const domain = domains[Math.floor(Math.random() * domains.length)];
                emailList.push(`${emailName}@${domain}`);
            }

            // Afficher les e-mails dans la div
        ...
        function exportEmails() {
            if (emailList.length === 0) {
                alert("Générer des e-mails avant d'exporter.");
                return;
            }
            ...
            // Créer un objet Blob avec les e-mails et spécifier le type de fichier comme text/plain
