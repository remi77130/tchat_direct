...
$(document).ready(function() {
 ...
 $('#pays_membre').on('change', function() {
	...
	if (country === 'Amérique') {
		...
		// Charge les villes d'Amérique
		$.ajax({
			url: 'get_cities_usa.php',
			method: 'GET',
			success: function(data) {
				var citySelect = $('#city');
				citySelect.empty();
				...
				if (data.length > 0) {
					data.forEach(function(city) {
						citySelect.append('<option value="' + city.city + '">' + city.city + ' (' + city.zip + ')</option>');
					});
				} else {
					$('#city-container').hide();
					citySelect.removeAttr('required');
		...
		// Cache la sélection de ville si le pays est différent
		$('#city-container').hide();
