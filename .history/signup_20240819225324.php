            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.length > 0) {
                        ...
                        citySelect.attr('required', true); // Ajoute l'attribut required
                    } else {
                        $('#city-container').hide(); // Cache la liste si aucune ville n'est trouvée
                        citySelect.removeAttr('required'); // Supprime l'attribut required
                    }
                }
            });
        } else {
            $('#city-container').hide();
            $('#ville_dpt').removeAttr('required'); // Supprime l'attribut required si le champ est caché
        }
    });
});
</script>
