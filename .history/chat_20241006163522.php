...
<script>
    // Liste simulée de profils
    const profils = [
        {
            avatar: 'img/favicon_2.png',
            username: 'JohnDoe',
            age: 25,
            department: 'Paris',
            ville: 'Paris'
    ...
    // Sélectionner la div qui contient les profils
    const container = document.getElementById('container_profil');

    // Fonction pour ajouter les profils dans la div
    function ajouterProfils() {
        profils.forEach(profil => {
            // Créer un élément div pour chaque profil
            const profilDiv = document.createElement('div');
            profilDiv.classList.add('profil');

            // Ajouter le contenu du profil dans la div
            profilDiv.innerHTML = `
                <img src="${profil.avatar}" alt="${profil.username}" class="avatar">
                <div class="info">
                    <span class="username">${profil.username}</span>
                    <span>Âge: ${profil.age}</span>
                    <span>Département: ${profil.department}</span>
                    <span>Ville: ${profil.ville}</span>
                </div>
            `;

            // Ajouter la div de chaque profil dans le container
            container.appendChild(profilDiv);
        });
    }

    // Appeler la fonction pour ajouter les profils
    ajouterProfils();
</script>
