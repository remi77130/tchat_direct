...
$query  = "SELECT * FROM users cities WHERE last_activity >= NOW() - INTERVAL 100 MINUTE";
        ...
        <?php foreach ($users as $user): ?>
            <div class="user">
                <div class="container_avatar_user">
                    <img class="avatar_user" src="<?= htmlspecialchars($user['avatar']) ?>" alt="Avatar de <?= htmlspecialchars($user['username']) ?>">
                </div>
                <p><?= htmlspecialchars($user['username']) ?> - <?= htmlspecialchars($user['age']) ?><?= htmlspecialchars($user['zip_code']) ?> ans (<?= htmlspecialchars($user['ville_users']) ?>)</p>
            </div>
        <?php endforeach; ?>
    </div>
</section>
...
// Mettre à jour la liste des utilisateurs toutes les 1000 secondes
setInterval(refreshUsers, 1000000);
