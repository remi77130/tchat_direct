...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
 ...
 // Gestion de l'avatar (par exemple)
 $avatarDestination = 'uploads/avatar_default.jpg'; // Image par défaut
 if (isset($_FILES['avatar']) && $_FILES['avatar']['error'] === UPLOAD_ERR_OK) {
     $avatar = $_FILES['avatar'];
     $avatarName = uniqid('', true) . "." . strtolower(pathinfo($avatar['name'], PATHINFO_EXTENSION));
     $avatarTmpName = $avatar['tmp_name'];
     $avatarDestination = 'C:/xampp/htdocs/salon/uploads/' . $avatarName;

     // Déplacement du fichier téléchargé
     if (!move_uploaded_file($avatarTmpName, $avatarDestination)) {
         error_log("Erreur lors du déplacement du fichier d'avatar.");
         $avatarDestination = 'uploads_users/avatar_default.jpg';
     }
 }
   ...
   $stmt->close();
    ...
    $avatarRelativePath = str_replace('C:/xampp/htdocs/salon/', '', $avatarDestination);

    $sql = "INSERT INTO users (username, age, department, ville_users, gender, avatar) VALUES ( ?, ?, ?, ?, ?, ?)";
    ...
    $stmt->bind_param("ssisss", $pseudo, $age, $department, $ville_users, $gender $avatarRelativePath);
