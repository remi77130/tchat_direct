        ...
        $('#department').on('input', function() {
            ...
            if (department.length > 0) {
                $.ajax({
                    url: 'get_cities.php',
                    method: 'GET',
                    data: { french_cities: zip_code },
                    success: function(data) {
                        var citySelect = $('#city');
                        citySelect.empty();
                        citySelect.append('<option value="zip_code">Sélectionnez une ville</option>');
                        
                        if (data.length > 0) {
                            data.forEach(function(city) {
                                citySelect.append('<option value="' + french_cities.zip_code + '">' + french_cities.zip_code + '</option>');
                            });
                            $('#city-container').show();
                        } else {
                            $('#city-container').hide();
