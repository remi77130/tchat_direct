  ...
  <div  class="counter" id="counter"></div>
    ...
    <input type="file" id="avatar" name="avatar" accept="image/*" >
                ...
                <input placeholder="Age" type="number" id="age" name="age" required>
                    ...
                    </div>  <label for="pays_membre">Pays :</label>
        <select id="pays_membre" name="pays_membre" required>
            <option value="">Sélectionnez un pays</option>
            <?php
            // Connexion à la base de données
            require 'connect_bdd.php';

            // Récupération des pays depuis la table 'pays'
            $sql = "SELECT nom_pays FROM pays";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    echo "<option value='" . htmlspecialchars($row['nom_pays']) . "'>" . htmlspecialchars($row['nom_pays']) . "</option>";
                }
            }
            ?>
        </select>
    ...
    <!-- Sélecteur de ville si le pays est Amérique -->
    <div id="city-container" style="display: none;">
        <label for="city">Ville (Amérique) :</label>
