...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['author']) && isset($_POST['comment'])) {
    ...
    $sql = "INSERT INTO comments (author, comment, parent_id) VALUES ('$author', '$comment', ".($parent_id ? "'$parent_id'" : "NULL").")";
...
<li><a href="#25"> Les tchats sans inscription sont-ils gratuits ?</a></li> 
    ...
    <h3>Laisser un petit commentaire</h3>
...
<?php
        ...
        function display_comments($parent_id = null, $level = 0) {
            ...
            $sql = "SELECT id, author, comment, created_at FROM comments WHERE parent_id ".($parent_id ? "= $parent_id" : "IS NULL")." ORDER BY created_at DESC";
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo "<button class='reply-button' data-comment-id='".$row['id']."'>Répondre</button>";
                    
                    // Formulaire de réponse
                    echo "<form class='reply-form' id='reply-form-".$row['id']."' style='display:none;' method='POST'>";
                    echo "<div><label for='author'>Nom :</label><input type='text' name='author' required></div>";
                    echo "<div><label for='comment'>Réponse :</label><textarea name='comment' rows='2' required></textarea></div>";
                    echo "<input type='hidden' name='parent_id' value='".$row['id']."'>";
                    echo "<button type='button' class='submit_reply' data-form-id='reply-form-".$row['id']."'>Envoyer</button>";
                    echo "</form>";
        ...
        // Appel de la fonction pour afficher les commentaires principaux
        display_comments();
        ?>
...
<script>
        document.getElementById('submit_comment').addEventListener('click', function () {
            ...
            .catch(error => {
                document.getElementById('response-message').innerHTML = 'Erreur lors de l\'envoi du formulaire.';
        ...
        document.querySelectorAll('.submit_reply').forEach(button => {
            button.addEventListener('click', function() {
