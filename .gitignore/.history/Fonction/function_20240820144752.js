...
function afficherVilleUtilisateur() {
    $.ajax({
        url: 'get_user_info.php', // Script PHP pour récupérer les infos de l'utilisateur
        method: 'GET',
        dataType: 'json',
        success: function(data) {
            if (data.ville) {
                $('#ville-utilisateur').text(data.ville); // Exemple d'affichage de la ville
            } else {
                $('#ville-utilisateur').text('Ville non spécifiée');
            }
        },
        error: function(xhr, status, error) {
            console.error("Erreur lors de la récupération des infos utilisateur : ", status, error);
        }
    });
}
