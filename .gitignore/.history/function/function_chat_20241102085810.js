...
/**
 * Charge la liste complète des utilisateurs au démarrage. Utilité : Met à jour la liste des utilisateurs connectés en temps réel.
 */
socket.on('users', function(users) { // Cette portion de code écoute l'événement 'users' et met à jour la liste des utilisateurs affichés sur l'interface. Voici l'explication détaillée ligne par ligne :

    $userlistContainer.empty(); // méthode jQuery qui vide l'élément de tout son contenu. Cela permet de réinitialiser la liste avant de la remplir à nouveau avec les données mises à jour.
 ...
 */
$(document).on('click', '.notification', (e) => { // Utilise jQuery pour attacher un écouteur d’événements 
                                                 // Fonction fléchée qui prend e (l'événement de clic) comme paramètre. Elle sera déclenchée chaque fois qu'un élément .notification est cliqué.de clic à tous les éléments ayant la classe .notification.
    $(e.currentTarget).remove(); // représente l’élément .notification qui a été cliqué.
                                //.remove() : Supprime cet élément de la page, ce qui signifie que la notification disparaît après le clic.

    let user_id = $(e.currentTarget).data('userid'); // extrait l’attribut data-userid de l’élément .notification, qui contient l’ID de l’utilisateur associé.
    let user = users[user_id];
    createChat(user); // Appelle la fonction createChat en passant user en argument. Cette fonction ouvre ou initialise une fenêtre de chat avec l'utilisateur sélectionné.
});
