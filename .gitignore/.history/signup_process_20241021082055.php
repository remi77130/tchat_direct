...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!in_array($gender, ['male', 'female', 'other'])) {
     ...
     // Validation des données
     if (strlen($mail) < 3 || strlen($mail) > 120) {
        die("Ton mail doit contenir entre 3 et 12 caractères.");
    }
   ...
   $stmt->close();
    ...
    $sql = "INSERT INTO users (username, avatar, age, department, ville_users, gender, mail) VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    $stmt->bind_param("ssisss", $pseudo, $avatarDestination, $age, $department, $ville_users, $gender, $mail);
