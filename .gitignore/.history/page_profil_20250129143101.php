...
<body class="bg-gray-100 font-sans">
    ...
    <textarea id="profileDescription" placeholder="Ajoute une courte description..." 
        maxlength="200" class="w-full p-2 border rounded mb-4"></textarea>
    ...
    <div id="profileDisplay" class="mt-4">
        ...
        <p id="descriptionText" class="text-gray-700 italic" title="Texte complet ici">Aucune description ajoutée.</p>
        </div>
</div>
<script>
    ...
    saveButton.addEventListener('click', () => {
        const text = profileDescription.value.trim(); // Récupère le texte sans espaces inutiles
        if (text) {
            descriptionText.textContent = text; // Met à jour le texte du profil
        } else {
            descriptionText.textContent = "Aucune description ajoutée."; // Texte par défaut si vide
        }
        profileDescription.value = ""; // Réinitialise le textarea après l'enregistrement
    });
</script>
